...
class PafNet:
    ...
    def stage_t(self, inputs, out_channel_num, name):
        ...
        #                        kernel_initializer=tf.truncated_normal_initializer(stddev=0.1),
        #                        bias_initializer=tf.truncated_normal_initializer(stddev=0.1))
        net = self.conv233(inputs=inputs, filters=128, padding='SAME', kernel_size=7, normalization=self.use_bn, name=name+'_conv1')
        net = self.conv233(inputs=net, filters=128, padding='SAME', kernel_size=7, normalization=self.use_bn, name=name+'_conv2')
        net = self.conv233(inputs=net, filters=128, padding='SAME', kernel_size=7, normalization=self.use_bn, name=name+'_conv3')
        net = self.conv233(inputs=net, filters=128, padding='SAME', kernel_size=7, normalization=self.use_bn, name=name+'_conv4')
        net = self.conv233(inputs=net, filters=128, padding='SAME', kernel_size=7, normalization=self.use_bn, name=name+'_conv5')
    ...
    def conv233(self, inputs, filters, padding, kernel_size, name, act=True, normalization=False):
        output1 = self.conv2(inputs=inputs, filters=128, padding='SAME', kernel_size=3, normalization=self.use_bn, name=name+'_1')
        output2 = self.conv2(inputs=output1, filters=128, padding='SAME', kernel_size=3, normalization=self.use_bn, name=name+'_2')
        output3 = self.conv2(inputs=output2, filters=128, padding='SAME', kernel_size=3, normalization=self.use_bn, name=name+'_3')
        return tf.concat([output1, output2, output3], 3)
